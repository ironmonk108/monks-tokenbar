...
import {i18n, MonksTokenBar, setting} from "../monks-tokenbar.js";

/**
 * Return a class that extends BaseRolls for either PF2e or SF2e system
 */
export function getRollsForPf2eBasedSystem(systemName) {
  return class extends BaseRolls {
    constructor() {
      super();
      // The config is always named PF2E, even for SF2E
      this._config = CONFIG['PF2E'];
    ...
    static activateHooks() {
      Hooks.on("preCreateChatMessage", (message, option, userid) => {
        let ctx = message.getFlag(systemName, 'context');
